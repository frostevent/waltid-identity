...
class SDMap(
    ...
    companion object {
        ...
        private fun buildSDArray(paths: List<List<PathToken>>, decoyMode: DecoyMode, decoys: Int): SDArray {
            ...
            val wildcard = groups[PathToken.Wildcard]?.let { fieldFromTails(it, decoyMode, decoys) }
            val maxIndex = groups.keys.filterIsInstance<PathToken.Index>().maxOfOrNull { it.value } ?: -1
            // Indices not explicitly listed inherit the wildcard if one is present, falling back
            // to a non-disclosable placeholder otherwise.
            val elements = (0..maxIndex).map { idx ->
                groups[PathToken.Index(idx)]?.let { fieldFromTails(it, decoyMode, decoys) }
                    ?: wildcard
                    ?: SDField(false)
            }
